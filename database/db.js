...
export function setStoryFlag(guildId, flag, value) {
  ...
  getDb().prepare('UPDATE guild_state SET story_flags = ? WHERE guild_id = ?')
    .run(JSON.stringify(state.story_flags), guildId);
}

export function updateLastVariants(guildId, variants) {
  getDb().prepare('UPDATE guild_state SET last_variants = ? WHERE guild_id = ?')
    .run(JSON.stringify(variants.slice(-6)), guildId);
}
...
export function saveDecision(guildId, sceneId, optionId, optionLabel, votes) {
  getDb().prepare(`
    INSERT INTO story_decisions (guild_id, scene_id, option_id, option_label, votes, decided_at)
    VALUES (?, ?, ?, ?, ?, ?)
...
export function createVote(guildId, sceneId, messageId, channelId, endsAt) {
  getDb().prepare(`
    INSERT OR REPLACE INTO active_votes (guild_id, scene_id, message_id, channel_id, ends_at, votes)
    VALUES (?, ?, ?, ?, ?, '{}')
...
export function castVote(guildId, userId, optionId) {
  ...
  getDb().prepare('UPDATE active_votes SET votes = ? WHERE guild_id = ?')
    .run(JSON.stringify(row.votes), guildId);
  return true;
}
...
export function addCharacter(guildId, name, role, faction, description, metAt) {
  const existing = getDb().prepare(
    'SELECT id FROM known_characters WHERE guild_id = ? AND name = ?'
  ).get(guildId, name);
  if (!existing) {
    getDb().prepare(`
      INSERT INTO known_characters (guild_id, name, role, faction, description, met_at)
      VALUES (?, ?, ?, ?, ?, ?)
